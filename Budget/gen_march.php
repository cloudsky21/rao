...
<img src="saaoblogo.jpg" style="float: left;">
				<p class="text-center">Republic of the Philippine<br><strong>PROVINCE OF LEYTE</strong><br>Local Government Unit-Tolosa, Leyte<br><br><strong>STATUS OF APPROPRIATIONS, ALLOTMENTS AND OBLIGATIONS <br>GENERAL FUND</strong><br>As of <label>March</label> <?PHP echo $_SESSION['budget'] ?></p>
</div>
	...
	<tr>
		...
		$result->execute(["MMO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_MMO  = $appr_MMO / 4;
				
				$appr2_MMO = $row['total_mooe'];
				$allotment2_MMO = $appr2_MMO / 4;
				
				$appr3_MMO = $row['total_co'];
				$allotment3_MMO = $appr3_MMO / 4;
				...
				/*----- Obligations ----- */
					...
					$obligation->execute([$_SESSION['budget'],"03"]);
	...
	<tr>
		...
		$result->execute(["SB",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_SB  = $appr_SB / 4;
				
				$appr2_SB = $row['total_mooe'];
				$allotment2_SB = $appr2_SB / 4;
				
				$appr3_SB = $row['total_co'];
				$allotment3_SB = $appr3_SB / 4;
	...
	<tr>
		...
		$result->execute(["MPDO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_mpdo  = $appr_mpdo / 4;
				
				$appr2_mpdo = $row['total_mooe'];
				$allotment2_mpdo = $appr2_mpdo / 4;
				
				$appr3_mpdo = $row['total_co'];
				$allotment3_mpdo = $appr3_mpdo / 4;
	...
	<tr>
		...
		$result->execute(["LCR",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_lcr  = $appr_lcr / 4;
				
				$appr2_lcr = $row['total_mooe'];
				$allotment2_lcr = $appr2_lcr / 4;
				
				$appr3_lcr = $row['total_co'];
				$allotment3_lcr = $appr3_lcr / 4;
	...
	<tr>
		...
		$result->execute(["MBO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_mbo  = $appr_mbo / 4;
				
				$appr2_mbo = $row['total_mooe'];
				$allotment2_mbo = $appr2_mbo / 4;
				
				$appr3_mbo = $row['total_co'];
				$allotment3_mbo = $appr3_mbo / 4;
	...
	<tr>
		...
		$result->execute(["MACCO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_macco  = $appr_macco / 4;
				
				$appr2_macco = $row['total_mooe'];
				$allotment2_macco = $appr2_macco / 4;
				
				$appr3_macco = $row['total_co'];
				$allotment3_macco = $appr3_macco / 4;
	...
	<tr>
		...
		$result->execute(["MTO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment2_mto = $appr2_mto / 4;
				
				$appr3_mto = $row['total_co'];
				$allotment3_mto = $appr3_mto / 4;
	...
	<tr>
		...
		$result->execute(["MASSO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_masso  = $appr_masso / 4;
				
				$appr2_masso = $row['total_mooe'];
				$allotment2_masso = $appr2_masso / 4;
				
				$appr3_masso = $row['total_co'];
				$allotment3_masso = $appr3_masso / 4;
	...
	<tr>
		...
		$result->execute(["General Services",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_gs  = $appr_gs / 4;
				
				$appr2_gs = $row['total_mooe'];
				$allotment2_gs = $appr2_gs / 4;
				
				$appr3_gs = $row['total_co'];
				$allotment3_gs = $appr3_gs / 4;
	...
	<tr>
		...
		$result->execute(["MHO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_mho  = $appr_mho / 4;
				
				$appr2_mho = $row['total_mooe'];
				$allotment2_mho = $appr2_mho / 4;
				
				$appr3_mho = $row['total_co'];
				$allotment3_mho = $appr3_mho / 4;
	...
	<tr>
		...
		$result->execute(["MSWD",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_mswd  = $appr_mswd / 4;
				
				$appr2_mswd = $row['total_mooe'];
				$allotment2_mswd = $appr2_mswd / 4;
				
				$appr3_mswd = $row['total_co'];
				$allotment3_mswd = $appr3_mswd / 4;
	...
	<tr>
		...
		$result->execute(["MAO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_mao  = $appr_mao / 4;
				
				$appr2_mao = $row['total_mooe'];
				$allotment2_mao = $appr2_mao / 4;
				
				$appr3_mao = $row['total_co'];
				$allotment3_mao = $appr3_mao / 4;
	...
	<tr>
		...
		$result->execute(["MEO",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_meo  = $appr_meo / 4;
				
				$appr2_meo = $row['total_mooe'];
				$allotment2_meo = $appr2_meo / 4;
				
				$appr3_meo = $row['total_co'];
				$allotment3_meo = $appr3_meo / 4;
	...
	<tr>
		...
		$result->execute(["MBE",$_SESSION['budget']]);
			...
			foreach($result as $row){
				...
				$allotment_mbe  = $appr_mbe / 4;
				
				$appr2_mbe = $row['total_mooe'];
				$allotment2_mbe = $appr2_mbe / 4;
				
				$appr3_mbe = $row['total_co'];
				$allotment3_mbe = $appr3_mbe / 4;
				...
				$all_list_name_20 = $row['propername'];
					...
					foreach($rs as $i){
						...
						echo '<tr>';
							...
							$obligation_20->execute([$_SESSION['budget'],"03"]);
				...
				$all_list_name_noneoffice = $row['propername'];
					...
					foreach($rs as $i){
						...
						echo '<tr>';
							...
							/* Allotment */
							$allotment_noneoffice = $i['programs'] / 4;
							...
							$obligation_noneoffice->execute([$_SESSION['budget'],"03"]);
