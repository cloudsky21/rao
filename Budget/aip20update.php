...
try
{
		$tbl_20_list = "20_list_".$_SESSION['budget']; 
		...
		//Delete the column from 20edf_CY
		$tbl_20edf = "20edf".$_SESSION['budget'];
		$alter_table = $db->prepare("ALTER TABLE `$tbl_20edf` DROP `$get_recorded`, DROP `$bal_recorded`");
		$alter_table->execute();
		
		//Delete the column from r20edf_CY
		$tbl_r20edf = "r20edf".$_SESSION['budget'];
...
$get_longname = htmlentities($_POST['longname-edit']);
	...
	if($_POST['sector'] == "GENERAL PUBLIC SERVICES"){
		...
		$tb_list1 = "20_list_".$_SESSION['budget'];
		...
		$tb_list2 = "20_list_".$_SESSION['budget'];
		...
		//Next Alter the Table of r20edf_CY
		$tb_r20 = "r20edf".$_SESSION['budget'];
...
$(document).ready(function(){
    $('#edit').on('show.bs.modal', function (e) {
        ...
        $.ajax({
			cache: false,
            type : 'post',
            url : 'edit20edf.php', //Here you will fetch records 
...
$(document).ready(function(){
    $('#delete').on('show.bs.modal', function (e) {
        ...
        $.ajax({
			cache: false,
            type : 'post',
            url : 'delete20edf.php', //Here you will fetch records 
            data :  'rowid='+ rowid, //Pass $id
            success : function(data){
...
<div class="container">
	<div style="margin-top: 100px;">
		...
		<div class="page-header">
			<h2>Update Table of  20% EDF</h2>
		</div>
  

			
				<label>Table: <STRONG>20&#37; EDF (<?PHP echo $_SESSION['budget'] ?>)</strong></label>&nbsp; &nbsp; <button class="btn btn-default btn-xs" data-toggle="modal" data-target="#addProgram"><span class="glyphicon glyphicon-plus"></span></button>  
