...
$result = $db->prepare("SELECT sum(total) as total FROM psgs WHERE year_transact = ?");
...
if(isset($_POST['delete_ps'])){
	...
	$result = $db->prepare("DELETE FROM psgs WHERE alobs = ?");
	$result->execute([$rec]);
...
//numbers	
	...
	$totalObligation = $post_salary +  	$post_pera +  $post_cloth + $post_yend + $post_gft + $post_mid + $post_pib + $post_gsis + $post_hdmf + $post_care + $post_ecc + $post_others;

	$query = "INSERT INTO psgs (
	year_transact, 
	month_transact, 
	day_transact, 
	alobs, 
...
<head>
	...
	<title>PS | General Services</title>
		...
		$(document).ready(function(){
			$('#infoModal').on('show.bs.modal', function (e) {
				...
				$.ajax({
					...
					type : 'post',
            url : 'details_gs_ps.php', //Here you will fetch records 
		...
		$(document).ready(function(){
			$('#editModal').on('show.bs.modal', function (e) {
				...
				$.ajax({
					...
					type : 'post',
            url : 'edit_gs_ps.php', //Here you will fetch records 
            data :  'rowid='+ rowid, //Pass $id
            success : function(data){
		...
		$(document).ready(function(){
			$('#deleteModal').on('show.bs.modal', function (e) {
				...
				$.ajax({
					...
					type : 'post',
            url : 'delete_gs_ps.php', //Here you will fetch records 
            data :  'rowid='+ rowid, //Pass $id
            success : function(data){
            $('.edit-data').html(data);//Show fetched data from database
        }
	...
	<div class="container">
		...
		<div style="margin-top: 100px;">
			...
			<div class="page-header">
				...
				<h2>General Services Office <small>Personal Services</small></h2>
			...
			<div class="modal fade" id="addObl" role="dialog">
				...
				<table class="table table-condensed table-striped table-hover">
					...
					<tbody>	
						...
						$result = $db->prepare("SELECT * FROM psgs WHERE year_transact = ?");
