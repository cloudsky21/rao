...
try
{
		$tbl_list = "lcpc_list_".$_SESSION['budget']; 
		...
		//Delete the column from 1sp_CY
		$tbl = "lcpc".$_SESSION['budget'];
		$alter_table = $db->prepare("ALTER TABLE `$tbl` DROP `$get_recorded`, DROP `$bal_recorded`");
		$alter_table->execute();
		
		//Delete the column from rgad_CY
		$tbl_r = "rlcpc".$_SESSION['budget'];
		...
		$edfCyTbl = $db->prepare("SELECT * FROM `$tbl_list`");
			...
			$count = $edfCyTbl->rowCount();
				...
				foreach($edfCyTbl as $list){
					...
					$edfcy = $db->prepare("SELECT `$codelist` as valueList FROM `$tbl`");
					$edfcy->execute();
				...
				$update_total = $db->prepare("UPDATE `$tbl` SET total = ?");
				$update_total->execute([$total]);
				}
				else{
				$update_total = $db->prepare("UPDATE `$tbl` SET total = 0");
...
try
{
	...
	$get_budget = "lcpc_list_".$_SESSION['budget'];
	...
	$tablename_20 = "lcpc".$_SESSION['budget'];
...
$get_longname = $_POST['longname-edit'];
	...
	if($_POST['sector'] == "GENERAL PUBLIC SERVICES"){
		...
		$tb_list1 = "lcpc_list_".$_SESSION['budget'];
		...
		$tb_list2 = "lcpc_list_".$_SESSION['budget'];
		...
		//Next Alter the Table of rlcpc_CY
		$tb_r = "rlcpc".$_SESSION['budget'];
...
$(document).ready(function(){
    $('#edit').on('show.bs.modal', function (e) {
        ...
        $.ajax({
			cache: false,
            type : 'post',
            url : 'editaip1lcpc.php', //Here you will fetch records 
...
$(document).ready(function(){
    $('#delete').on('show.bs.modal', function (e) {
        ...
        $.ajax({
			cache: false,
            type : 'post',
            url : 'deleteaip1lcpc.php', //Here you will fetch records 
            data :  'rowid='+ rowid, //Pass $id
            success : function(data){
...
<nav class="navbar navbar-default navbar-fixed-top">
	...
	<li><a  class="dropdown-toggle" data-toggle="dropdown" href="#">Settings
		...
		<ul class="dropdown-menu">
			...
			<li><a href="aip1iralcpc">Back-to-AIP <strong>IRA-LCPC</strong></a></li>
			<li><a href="aip1lcpcsetup">Setup Table</a></li>
			<li><a href="aip1lcpcupdate">Update Column</a></li>
...
<div class="container">
	<div style="margin-top: 100px;">
		...
		<div class="page-header">
			<h2>Update Table of  1% IRA - LCPC</h2>
		</div>
  

			
				<label>Table: <STRONG>IRA - LCPC (<?PHP echo $_SESSION['budget'] ?>)</strong></label>&nbsp; &nbsp; <button class="btn btn-default btn-xs" data-toggle="modal" data-target="#addProgram"><span class="glyphicon glyphicon-plus"></span></button>  
						...
						<table class="table table-condensed table-hover">
							...
							<tbody>
								<?PHP 
										...
										$current_year = "lcpc_list_".$_SESSION['budget'];
