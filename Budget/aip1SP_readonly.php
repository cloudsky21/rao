...
<div class="page-header">
	...
	<small>Note: If you saw that "<strong>Total</strong>" has value even if the table is empty. Just Proceed, it will adjust if you add a value to the program.</small>
  </div>
  
</div>
<form action="" method="POST">
<label>OFFICE &#47; SPECIAL PURPOSE APPROPRIATION: <STRONG>1% Senior Citizen & Persons With Disability</strong></label>  
...
<thead>
	...
	<th>Balance<br>&#40;3&#41;</th>
		...
		<?PHP 
		$tablename = "1sp_list_".$_SESSION['budget']; //1sp_list_(current Year) table
		...
		foreach($edfCyTbl as $list){
			...
			$table2 = "1sp".$_SESSION['budget']; //1sp_CY table
			$codelist = $list['code'];
			...
			$tablerao = "r1sp".$_SESSION['budget'];
			$redfcy = $db->prepare("SELECT SUM($codelist) as total FROM `$tablerao`");
			$redfcy->execute();
			
				foreach($redfcy as $redfcy_total){
					
					$totalb = $r['valueList'] - $redfcy_total['total'];
