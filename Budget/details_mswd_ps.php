...
$result = $db->prepare("SELECT * FROM psmswd WHERE alobs = ? AND year_transact = ?");
...
$ecc = $row['ecc']; 
$subsistence = $row['subsistenceAllowance'];
...
?>
			...
			<ul class="list-group">
				...
				if($subsistence !="0.00"){ echo'<li class="list-group-item">Subsistence Allowance<span class="badge">'.number_format($subsistence,2).'</span></li>';} else{} 
