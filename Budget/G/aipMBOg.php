...
$isLoaded = check_if_loaded_mbo_aip();
		if ($isLoaded == '1'){
	
			header("location: aipMBO_readonly.php");
			}
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	$dept_code = "MBO";
	$dept_head = "LOIDA A. PALAÑA";
	$dept_name = "Municipal Budget Office";
	...
	$query = "INSERT INTO aip (Year, departments, deptName, headOfOffice, salaries, PERA, RA, TA, clothing_allowance, year_end, cash_gift, mid_year, pib, life_retirement, pag_ibig, philHealth, ecc, other_personal, total_ps,
								tev, training_seminars, telephone_telegraph, office_supplies, others, total_mooe, co, total_co) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	
	if ($result = mysqli_prepare($db, $query)){
		
	mysqli_stmt_bind_param($result,"sssssssssssssssssssssssssss", $budy, $dept_code, $dept_name, $dept_head, $post_salary, $post_pera, $post_ra, $post_ta, $post_clothing, $post_year_end, $post_cash_gift, $post_mid_year, $post_pib, $post_gsis, $post_hdmf, $post_philHealth, $post_ecc, $post_opb, $total_ps, $post_tev, $post_training, $post_telephone, $post_office_supplies, $post_other_maintenance, $total_mooe, $post_capital_outlay, $total_coe);
