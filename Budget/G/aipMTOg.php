...
$isLoaded = check_if_loaded_mto_aip();
		if ($isLoaded == '1'){
	
			header("location: aipMTO_readonly.php");
			}
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	$dept_name = "Municipal Treasurers Office";
	...
	$post_fed_ins = mysqli_real_escape_string($db, $_POST['fedility-insurance-premium']);
	$post_account = mysqli_real_escape_string($db, $_POST['accountable-forms']);
	$post_office_supplies = mysqli_real_escape_string($db,$_POST['office-supplies']);
	$post_it = mysqli_real_escape_string($db, $_POST['it-equipment']);
	...
	$total_ps = $post_salary + $post_pera + $post_ra + $post_ta + $post_clothing + $post_year_end + $post_cash_gift + $post_mid_year + $post_pib + $post_gsis + $post_hdmf + $post_philHealth + $post_ecc + $post_tlb + $post_opb; 
	$total_mooe = $post_tev +  $post_training + $post_postage + $post_telephone + $post_fed_ins + $post_account + $post_office_supplies + $post_it + $post_other_maintenance;
	$total_coe = $post_capital_outlay + 0;

	
	$query = "INSERT INTO aip (Year, departments, deptName, headOfOffice, salaries, PERA, RA, TA, clothing_allowance, year_end, cash_gift, mid_year, pib, life_retirement, pag_ibig, philHealth, ecc, terminal_leave, other_personal, total_ps,
								tev, training_seminars, postage, telephone_telegraph, fidelity_bond, accountable_forms, office_supplies, repairs_it_equip, others, total_mooe, co, total_co) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	
	if ($result = mysqli_prepare($db, $query)){
		
	mysqli_stmt_bind_param($result,"ssssssssssssssssssssssssssssssss", $budy, $dept_code, $dept_name, $dept_head, $post_salary, $post_pera, $post_ra, $post_ta, $post_clothing, $post_year_end, $post_cash_gift, $post_mid_year, $post_pib, $post_gsis, $post_hdmf, $post_philHealth, $post_ecc, $post_tlb, $post_opb, $total_ps, $post_tev, $post_training, $post_postage, $post_telephone, $post_fed_ins, $post_account, $post_office_supplies, $post_it, $post_other_maintenance, $total_mooe, $post_capital_outlay, $total_coe);
...
<thead>
	...
	<th>Budget Year Proposed<br>&#40;<?PHP echo $_SESSION['budget']; ?>&#41;</th>
		...
		<tr>
			...
			<td>&nbsp;</td>
			<td><input type="number" id="tlb" name="tlb" class="w3-input" step="any" value="0.00"></td>
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 05 &#32; 030</td>
			<td><input type="number" id="telephone" name="telephone" class="w3-input" step="any" value="0.00"></td>
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 16 &#32; 020</td>
			<td><input type="number" id="fedility-insurance-premium" name="fedility-insurance-premium" class="w3-input" step="any" value="0.00"></td>
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 03 &#32; 020</td>
			<td><input type="number" id="accountable-forms" name="accountable-forms" class="w3-input" step="any" value="0.00"></td>
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 13 &#32; 050</td>
			<td><input type="number" id="it-equipment" name="it-equipment" class="w3-input" step="any" value="0.00"></td>
