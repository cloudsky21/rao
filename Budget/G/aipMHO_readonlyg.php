...
mysqli_set_charset($db,"utf8");
	...
	$post_hwb = mysqli_real_escape_string($db, $_POST['hwb']);
	...
	$post_motm = mysqli_real_escape_string($db,$_POST['vehicle-maintenance']);
	...
	$total_ps = $post_salary + $post_pera + $post_ra + $post_ta + $post_clothing + $post_year_end + $post_cash_gift + $post_mid_year + $post_pib + $post_gsis + $post_hdmf + $post_philHealth + $post_ecc + $post_hwb + $post_opb; 
	$total_mooe = $post_tev +  $post_training + $post_telephone + $post_motm + $post_office_supplies + $post_other_maintenance;
	$total_coe = $post_capital_outlay + 0;

	
	$query = "UPDATE aip SET salaries = ?, PERA = ?, RA = ?, TA = ?, clothing_allowance = ?, year_end = ?, cash_gift = ?, 
							 mid_year = ?, pib = ?, life_retirement = ?, pag_ibig = ?, philhealth = ?, ecc = ?, healthWorkersBenefits = ?, other_allowance_bonuses = ?, total_ps = ?, 
							 tev = ?, training_seminars = ?, telephone_telegraph = ?, office_supplies = ?, motor_vehicle_maint = ?, others = ?, total_mooe = ?, co = ?, total_co = ? WHERE departments = 'MHO' && Year = ?  ";
	
	if ($result = mysqli_prepare($db, $query)){
		
	mysqli_stmt_bind_param($result,"ssssssssssssssssssssssssss",$post_salary, $post_pera, $post_ra, $post_ta, $post_clothing, $post_year_end, $post_cash_gift, $post_mid_year, $post_pib, $post_gsis, $post_hdmf, $post_philHealth, $post_ecc, $post_hwb, $post_opb, $total_ps, $post_tev, $post_training, $post_telephone,  $post_office_supplies, $post_motm, $post_other_maintenance, $total_mooe, $post_capital_outlay, $total_coe, $budy);
...
<title>AIP | Municipal Health Office</title>
<style>
html, body {
...
<div id="container">
	<div class="wrap">
		<ul>
			...
			<li><a href="aipMHO.php"  class="active">MHO</a></li>
...
<div id="mySidenav" class="sidenav">
  ...
  <a href="personal_services_mho_all.php">Personnel Services</a>
  <a href="mooe_mho_all.php">Maint. &amp; Other Operating Expenses</a>
  <a href="co_mho_all.php">Capital Outlay</a>
...
<label>OFFICE &#47; SPECIAL PURPOSE APPROPRIATION: <STRONG>OFFICE OF THE MUNICIPAL HEALTH OFFICER</strong></label>  
...
<thead>
	...
	<th>Balance<br>&#40;4&#41;</th>
		...
		<tr>
			<td>Health Workers Benefits</td>
			<td>&nbsp;</td>
			<?PHP
			$ec = floatval(select_aip_hwb());
			echo '<td>'.number_format($ec,2).'</td>';
			echo '<td><input type="number" id="hwb" name="hwb" class="w3-input" value="'.$ec.'" tabindex="14" step="any"></td>';
			echo '<td><input type="text" class="w2-input" readonly value="'.number_format(select_aip_hwb_bal(),2).'"></td>';
			?>
		</tr>
		...
		<td>Other Personnel Benefits</td>
			...
			echo '<td><input type="number" id="opb" name="opb" class="w3-input" value="'.$opb.'" tabindex="15" step="any"></td>';
		...
		<tr>
			...
			echo '<td><input type="number" id="tev" name="tev" class="w3-input" value="'.$tv.'" tabindex="16" step="any"></td>';
		...
		<tr>
			...
			echo '<td><input type="number" id="training" name="training" class="w3-input" value="'.$tra.'" tabindex="17 step="any"></td>';
		...
		<tr>
			...
			echo '<td><input type="number" id="office-supplies" name="office-supplies" class="w3-input" value="'.$off.'" tabindex="22" step="any"></td>';
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 13 &#32; 060</td>
			<?PHP
			$motm = floatval(select_aip_motm());
			echo '<td>'.number_format($motm,2).'</td>';
			echo '<td><input type="number" id="vehicle-maintenance" name="vehicle-maintenance" class="w3-input" value="'.$motm.'" tabindex="23" step="any"></td>';
			echo '<td><input type="text" class="w2-input" readonly value="'.number_format(select_aip_motm_bal(),2).'"></td>';
			?>
			</tr>
		...
		<tr>
			...
			echo '<td><input type="number" id="telephone" name="telephone" class="w3-input" value="'.$phonegraph.'" tabindex="24" step="any"></td>';
		...
		<tr>
			...
			echo '<td><input type="number" id="other-maintenance" name="other-maintenance" class="w3-input" value="'.$ot.'" tabindex="25" step="any"></td>';
		...
		<tr>
			...
			echo '<td><input type="number" id="capital-outlay" name="capital-outlay" class="w3-input" value="'.$coo.'" tabindex="26" step="any"></td>';
