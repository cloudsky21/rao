...
$isLoaded = check_if_loaded_lcr_aip();
		if ($isLoaded == '1'){
	
			header("location: aipLCR_readonly.php");
			}
...
mysqli_set_charset($db,"utf8");
	...
	$dept_code = "LCR";
	$dept_head = " ";
	$dept_name = "Local Civil Registrar";
	...
	$query = "INSERT INTO aip (Year, departments, deptName, headOfOffice, salaries, PERA, RA, TA, clothing_allowance, year_end, cash_gift, mid_year, pib, life_retirement, pag_ibig, philHealth, ecc, other_personal, total_ps,
								tev, training_seminars, postage, office_supplies, others, total_mooe, co, total_co) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
