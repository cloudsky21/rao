...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	$total_mooe = $post_tev +  $post_training +  $post_telephone + $post_postage + $post_fedility_premium + $post_office_supplies + $post_other_maintenance;
	...
	$query = "UPDATE aip SET salaries = ?, PERA = ?, RA = ?, TA = ?, clothing_allowance = ?, year_end = ?, cash_gift = ?, mid_year = ?, pib = ?, life_retirement = ?, pag_ibig = ?, philHealth = ?, ecc = ?, terminal_leave = ?, other_personal = ?, total_ps = ?,
								tev = ?, training_seminars = ?, telephone_telegraph = ?, postage = ?, fidelity_bond = ?, office_supplies = ?, advertising_expenses = ?, others = ?, co = ?, total_co = ? WHERE Year = ? && departments = 'SB' ";
	
	if ($result = mysqli_prepare($db, $query)){
		
	mysqli_stmt_bind_param($result,"sssssssssssssssssssssssssss", $post_salary, 
	$post_pera, 
	$post_ra, 
	$post_ta, 
	$post_clothing, 
...
echo '<label>OFFICE &#47; SPECIAL PURPOSE APPROPRIATION: <strong>OFFICE OF THE SANGGUNIANG BAYAN</strong></label><br> ';
echo '<table>';
echo '<thead>';
...
echo '</thead>';
	...
	echo '	<tr>';
		...
		echo '<tr>';
		echo '<td>Advertising Expenses</td>';
		echo '<td>5&#32; 02 &#32; 99 &#32; 010</td>';
		echo '<td>'.number_format($ads,2).'</td>';
		echo '<td><input type="number" id="ads" name="ads" class="w3-input" step=0.01 min=0 value="'.$ads.'"></td>';
		echo '<td><input type="text" class="w2-input" readonly value="'.number_format(select_aip_ads_bal(),2).'"></td>';
		echo '</tr>';
