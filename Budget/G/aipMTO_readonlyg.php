...
mysqli_set_charset($db,"utf8");
	...
	$post_tlb = mysqli_real_escape_string($db, $_POST['tlb']);
	...
	$post_fedility = mysqli_real_escape_string($db,$_POST['fedility-insurance-premium']);
	$post_acc = mysqli_real_escape_string($db,$_POST['accountable-forms']);
	$post_office_supplies = mysqli_real_escape_string($db,$_POST['office-supplies']);
	$post_it_equipment = mysqli_real_escape_string($db,$_POST['it-equipment']);
	...
	$total_mooe = $post_tev +  $post_training +  + $post_postage + $post_telephone + $post_fedility + $post_acc + $post_office_supplies + $post_it_equipment +$post_other_maintenance;
	...
	total_ps = ?, 
	 ...
	 total_mooe = ?, 
	 co = ?, 
	 total_co = ? WHERE departments = 'MTO' && Year = ?  ";
	
	if ($result = mysqli_prepare($db, $query)){
		
	mysqli_stmt_bind_param($result,"sssssssssssssssssssssssssssss",
	$post_salary, 
	$post_pera, 
	$post_ra, 
	$post_ta, 
...
<title>AIP | Municipal Treasurers Office</title>
<style>
html, body {
...
<div id="container">
	<div class="wrap">
		<ul>
			...
			<li><a href="aipMTO.php"  class="active">MTO</a></li>
...
<div id="mySidenav" class="sidenav">
  ...
  <a href="personal_services_mto_all.php">Personnel Services</a>
  <a href="mooe_mto_all.php">Maint. &amp; Other Operating Expenses</a>
...
<label>OFFICE &#47; SPECIAL PURPOSE APPROPRIATION: <STRONG>OFFICE OF THE MUNICIPAL TREASURER</strong></label>  
...
<thead>
	...
	<th>Balance<br>&#40;4&#41;</th>
		...
		<tr>
			...
			<td>&nbsp;</td>
			<?PHP
			$tlb = floatval(select_aip_tl());
			echo '<td>'.number_format($tlb,2).'</td>';
			echo '<td><input type="number" id="tlb" name="tlb" class="w3-input" value="'.$tlb.'" tabindex="14" step="any"></td>';
		...
		<tr>
			...
			echo '<td><input type="number" id="postage" name="postage" class="w3-input" step="any" value="'.$po.'" tabindex="18"></td>';
		...
		<tr>
			...
			echo '<td><input type="number" id="telephone" name="telephone" class="w3-input" value="'.$phonegraph.'" tabindex="19" step="any"></td>';
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 16 &#32; 020</td>
			<?PHP 
			$fed = floatval(select_aip_fidelity());
			echo '<td>'.number_format($fed,2).'</td>';
			echo '<td><input type="number" id="fedility-insurance-premium" name="fedility-insurance-premium" class="w3-input" value="'.$fed.'" tabindex="20" step="any"></td>';
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 03 &#32; 020</td>
			<?PHP 
			$account = floatval(select_aip_accountable());
			echo '<td>'.number_format($account,2).'</td>';
			echo '<td><input type="number" id="accountable-forms" name="accountable-forms" class="w3-input" value="'.$account.'" tabindex="21" step="any"></td>';
			echo '<td><input type="text" class="w2-input" readonly value="'.number_format(select_aip_accountable_bal(),2).'"></td>';
			?>
		</tr>
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 13 &#32; 050</td>
			<?PHP
			$it = floatval(select_aip_it());
			echo '<td>'.number_format($it,2).'</td>';
			echo '<td><input type="number" id="it-equipment" name="it-equipment" class="w3-input" value="'.$it.'" tabindex="23" step="any"></td>';
			echo '<td><input type="text" class="w2-input" readonly value="'.number_format(select_aip_it_bal(),2).'"></td>';
			?>
		</tr>
		...
		<tr>
			...
			echo '<td><input type="number" id="other-maintenance" name="other-maintenance" class="w3-input" value="'.$ot.'" tabindex="24" step="any"></td>';
		...
		<tr>
			...
			echo '<td><input type="number" id="capital-outlay" name="capital-outlay" class="w3-input" value="'.$coo.'" tabindex="25" step="any"></td>';
