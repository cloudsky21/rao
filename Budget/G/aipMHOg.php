...
$isLoaded = check_if_loaded_mho_aip();
		if ($isLoaded == '1'){
	
			header("location: aipMHO_readonly.php");
			}
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	$total_mooe = $post_tev +  $post_training + $post_telephone + $post_office_supplies + $post_motor_vehicle + $post_other_maintenance;
	$total_coe = $post_capital_outlay + 0;

	
	$query = "INSERT INTO aip (Year, departments, deptName, headOfOffice, salaries, PERA, RA, TA, clothing_allowance, year_end, cash_gift, mid_year, pib, life_retirement, pag_ibig, philHealth, ecc, healthWorkersBenefits, other_personal, total_ps,
								tev, training_seminars, telephone_telegraph, office_supplies, motor_vehicle_maint, others, total_mooe, co, total_co) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	
	if ($result = mysqli_prepare($db, $query)){
		
	mysqli_stmt_bind_param($result,"sssssssssssssssssssssssssssss", $budy, $dept_code, $dept_name, $dept_head, $post_salary, $post_pera, $post_ra, $post_ta, $post_clothing, $post_year_end, $post_cash_gift, $post_mid_year, $post_pib, $post_gsis, $post_hdmf, $post_philHealth, $post_ecc, $post_hwb, $post_opb, $total_ps, $post_tev, $post_training, $post_telephone, $post_office_supplies, $post_motor_vehicle, $post_other_maintenance, $total_mooe, $post_capital_outlay, $total_coe);
...
<thead>
	...
	<th>Budget Year Proposed<br>&#40;<?PHP echo $_SESSION['budget']; ?>&#41;</th>
		...
		<tr>
			...
			<td>&nbsp;</td>
			<td><input type="number" id="hwb" name="hwb" class="w3-input" step="any" value="0.00"></td>
		...
		<tr>
			...
			<td>5&#32; 02 &#32; 13 &#32; 060</td>
			<td><input type="number" id="vehicle-maintenance" name="vehicle-maintenance" class="w3-input" step="any" value="0.00"></td>
