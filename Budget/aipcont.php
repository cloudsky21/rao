...
$checkTableExist = get_table_exist_cont();


if($checkTableExist == '0'){

$isLoaded = check_if_loaded_cont();
		if ($isLoaded > 0){
	
			
			header("location: aipcont_readonly");
			exit();
			}
...
$sig = "Erwin C. Ocaña";
	...
	if(($rwc = $result->rowCount())=="1"){
		...
		unset($_POST);
		header("location: ".$_SERVER['PHP_SEF']);
...
<label>OFFICE &#47; SPECIAL PURPOSE APPROPRIATION: <STRONG>Continuing Programs</strong></label>  
