...
$result = $db->prepare("SELECT * FROM psmpdo WHERE alobs = ? AND year_transact = ?");
...
$ecc = $row['ecc']; 
$olb = $row['other_allowance_bonuses']; 
$dtotal = $row['total'];
...
?>
			...
			<ul class="list-group">
				...
				if($olb !="0.00"){ echo'<li class="list-group-item">Other Bonuses and Allowances<span class="badge">'.number_format($opb,2).'</span></li>';} else{} 
				?>
			</ul>
			</div>	
