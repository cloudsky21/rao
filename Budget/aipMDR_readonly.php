...
if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		if(!empty($_POST['amount_adjusted'])){	
			...
			$amountUpdate = $_POST['amount_adjusted'];
				...
				//Loop to update mdr table
					foreach($_POST['idnumber'] as $key=>$value){
						
					$tb = "mdr".$_SESSION['budget'];	
	...
	?>	
	  <li><a  class="dropdown-toggle" data-toggle="dropdown" href="#">Settings
		...
		<ul class="dropdown-menu">
			...
			<li><a href="aipmdrsetup">Setup Table</a></li>
			<li><a href="aipmdrupdate">Update Column</a></li>
...
<thead>
	...
	<th>Balance<br>&#40;3&#41;</th>
		...
		<?PHP
try{		
		$tablename = "mdr_list_".$_SESSION['budget']; //mdr_list_(current Year) table
		$mdrCyTbl = $db->prepare("SELECT * FROM `$tablename`");
		$mdrCyTbl->execute();
		
		foreach($mdrCyTbl as $list){
			
			
			
			$table2 = "mdr".$_SESSION['budget']; //mdr_CY table
			$codelist = $list['code'];
