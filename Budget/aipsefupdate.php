...
try
{
		$tbl_list = "sef_list_".$_SESSION['budget']; 
		...
		//Delete the column from sef_CY
		$tbl_sef = "sef".$_SESSION['budget'];
		$alter_table = $db->prepare("ALTER TABLE `$tbl_sef` DROP `$get_recorded`, DROP `$bal_recorded`");
		$alter_table->execute();
		
		
		//Delete the column from rgad_CY
		$tbl_r = "rsef".$_SESSION['budget'];
		...
		$alter_table->execute();
			...
			$count = $edfCyTbl->rowCount();
				...
				foreach($edfCyTbl as $list){
					...
					$edfcy = $db->prepare("SELECT `$codelist` as valueList FROM `$tbl_sef`");
					$edfcy->execute();
				...
				$update_total = $db->prepare("UPDATE `$tbl_sef` SET total = ?");
				$update_total->execute([$total]);
				}
				else{
				$update_total = $db->prepare("UPDATE `$tbl_sef` SET total = 0");
...
try
{
	...
	$get_budget = "sef_list_".$_SESSION['budget'];
	$insert_to_db = $db->prepare("INSERT INTO `$get_budget` (code, propername, type, others) VALUES (?,?,?,?)");
	$insert_to_db->execute([$_POST['new_code'], $_POST['new_name'], $_POST['for-type'], $_POST['others']]);
	...
	$tablename_20 = "sef".$_SESSION['budget'];
	...
	$tabler = "rsef".$_SESSION['budget'];
	$query = "ALTER TABLE $tabler ADD `$addcolumn` DECIMAL(20,2) NOT NULL AFTER `$last_column_r`"; 
	$altertable = $db->prepare($query);
	$altertable->execute();
...
$get_longname = htmlentities($_POST['longname-edit']);
	...
	try{
		...
		$tb_list1 = "sef_list_".$_SESSION['budget'];
		...
		$tb_list2 = "sef_list_".$_SESSION['budget'];
		$rs = $db->prepare("UPDATE `$tb_list2` SET code = ?, propername = ?, type = ?, others=? WHERE id = ?");
		$rs->execute([$get_shortname, $get_longname, $_POST['for-type'], $_POST['others'], $get_id]);
		
		//Next Alter the Table of 20edf_CY
		$tb_20 = "sef".$_SESSION['budget'];
		...
		//Next Alter the Table of rgad_CY
		$tb_r = "rsef".$_SESSION['budget'];
		$rs = $db->prepare("ALTER TABLE `$tb_r` CHANGE `$old_column` `$get_shortname` DECIMAL(20,2) NOT NULL;");
		$rs->execute();
		
		$rs = $db->prepare("UPDATE $tb_r SET code = ? WHERE code = ?");
...
$(document).ready(function(){
    $('#edit').on('show.bs.modal', function (e) {
        ...
        $.ajax({
			cache: false,
            type : 'post',
            url : 'editaipsef.php', //Here you will fetch records 
...
$(document).ready(function(){
    $('#delete').on('show.bs.modal', function (e) {
        ...
        $.ajax({
			cache: false,
            type : 'post',
            url : 'deleteaipsef.php', //Here you will fetch records 
            data :  'rowid='+ rowid, //Pass $id
            success : function(data){
...
<nav class="navbar navbar-default navbar-fixed-top">
	...
	<li><a  class="dropdown-toggle" data-toggle="dropdown" href="#">Settings
		...
		<ul class="dropdown-menu">
			...
			<li><a href="aipSEF">Back-to-AIP <strong>SEF</strong></a></li>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
				...
				<div class="form-group">
				<label for="others" class="control-label col-sm-4">Others: </label>
					<div class="col-sm-8">	
...
<div class="modal fade" id="edit" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
				<button type="submit" name="edit_program_btn" class="btn btn-primary">Edit</button>
            </div>
			</form>
        </div>
    </div>
...
<div class="container">
	<div style="margin-top: 100px;">
		...
		<div class="page-header">
			<h2>Update Table of  Special Education Fund</h2>
		</div>
  

			
				<label>Table: <STRONG>Special Education Fund (<?PHP echo $_SESSION['budget'] ?>)</strong></label>&nbsp; &nbsp; <button class="btn btn-default btn-xs" data-toggle="modal" data-target="#addProgram"><span class="glyphicon glyphicon-plus"></span></button>  
						...
						<table class="table table-condensed table-hover">
							...
							<tbody>
								<?PHP 
										...
										/*Check if set in sef_list Current year*/
										$current_year = "sef_list_".$_SESSION['budget'];
										...
										echo '<td><input type="hidden" name="others" value="'.$row['others'].'">'.$row['others'].'</td>';
