...
$get_table_check = get_table_exist_noneoffice();


if($get_table_check == 1){


	$tablename = "noneoffice_list_".$_SESSION['budget'];
	$table_aip = "noneoffice".$_SESSION['budget'];
	$table_rao = "rnoneoffice".$_SESSION['budget'];
...
try
{
	...
	$insert_to_db->execute([$_POST['for-code'], $_POST['for-name'], $_POST['sector'], $_POST['ref'], $_POST['for-type']]);
	...
	$query = "ALTER TABLE `$tablename` ADD `$addcolumn` DECIMAL(20,2) NOT NULL AFTER `$last_column`, ADD `$addcolumn_bal` DECIMAL(20,2) NOT NULL AFTER $last_column"; 
...
<title>AIP | None - Office Setup</title>
	...
	<div style="margin-top: 100px;">
		...
		<div class="page-header">
			<h2>Setup None Office</h2>
				...
				<label>Table: <STRONG>None-Office Programs from previous year</strong> </label>  
						...
						<table  class="table table-condensed table-hover">
							...
							<tbody>
								...
								try{
									...
									$result = $db->prepare("SELECT * FROM noneoffice_list_".$sget."");
									$result->execute();
									
									foreach($result as $row){
										
										
										/*Check if set in 1sp_list Current year*/
