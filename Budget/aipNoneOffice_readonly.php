...
if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		if(!empty($_POST['amount_adjusted'])){	
			...
			$amountUpdate = $_POST['amount_adjusted'];
				...
				//Loop to update noneoffice table
					foreach($_POST['idnumber'] as $key=>$value){
						
					$tb = "noneoffice".$_SESSION['budget'];	
...
<label>OFFICE &#47; SPECIAL PURPOSE APPROPRIATION: <STRONG>NONE - OFFICE</strong></label>  
...
<?PHP
		...
		$tablename = "noneoffice_list_".$_SESSION['budget']; //mdr_list_(current Year) table
		...
		foreach($mdrCyTbl as $list){
			...
			$table2 = "noneoffice".$_SESSION['budget']; //noneoffice_CY table
			$codelist = $list['code'];
			...
			$tablerao = "rnoneoffice".$_SESSION['budget'];
